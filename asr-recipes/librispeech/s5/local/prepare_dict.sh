...
if [ ! -f $lexicon_raw_nosil ]; then
  ...
  phonemize -l no -p " " -w "" -s "" --language-switch remove-flags $vocab_only_words > $dst_dir/tmp_prons
  # phonemize -l no --language-switch remove-flags $vocab_only_words > $dst_dir/tmp_prons
...
echo "FInished reading prepare dict"
